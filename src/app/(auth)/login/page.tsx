...
export default function LoginPage() {
  ...
  const [pendingRole, setPendingRole] = useState<UserRole>(demoRole)
  const [signingIn, setSigningIn] = useState(false)

  useEffect(() => {
    if (!loading && user) {
      router.replace(ROLE_HOME[user.role] ?? '/sale')
    }
  }, [user, loading, router])

  // Sync pendingRole when demoRole changes from outside (e.g. DemoRoleSwitcher)
  useEffect(() => {
      ...
      <main className="min-h-screen flex items-center justify-center bg-slate-50 px-4">
        <div className="w-full max-w-md">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-4">
            ...
            <button
              onClick={() => setDemoRole(pendingRole)}
            ...
            >
              เข้าสู่ระบบ Demo ในฐานะ {ROLE_LABELS[pendingRole]}
            </button>
      ...
      <div className="flex-1 bg-white flex items-center justify-center p-8 lg:p-16">
        <div className="w-full max-w-sm">
          ...
          <button
            ...
            disabled={signingIn}
            className="w-full flex items-center justify-center gap-3 px-4 py-3.5 rounded-xl border-2 border-gray-200 bg-white hover:border-freshket-300 hover:bg-freshket-100/40 text-gray-800 font-bold text-sm transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-freshket-300 disabled:opacity-60 disabled:cursor-not-allowed"
          >
            {signingIn ? (
              <div className="size-5 border-2 border-gray-300 border-t-gray-700 rounded-full animate-spin shrink-0" />
            ) : (
            ...
            {signingIn ? 'กำลังเข้าสู่ระบบ…' : 'Sign in with Google'}
          </button>

...
// ─── Key Take Away / Reflect Knowledge ────────────────────────────────────────
// What a learner wrote after finishing a course, keyed `${uid}_${courseId}` to
// match the real `takeaways` collection. Lengths vary on purpose: the manager
// view has to hold up for a one-line reply and for someone who writes six
// paragraphs, and only real-shaped text shows whether it does.
export const MOCK_TAKEAWAYS: Record<string, string> = {
  // Long — the case that forced the read-more modal.
  [`mock-sale-01_${C[0].id}`]:
    'หลังเรียนจบได้เข้าใจว่าลูกค้าร้านอาหารไม่ได้เลือกซัพพลายเออร์จากราคาอย่างเดียว แต่ดูความสม่ำเสมอของคุณภาพและการส่งตรงเวลาเป็นหลัก\n\n' +
    'สิ่งที่จะเอาไปใช้จริง คือเวลาเข้าไปคุยกับเชฟหรือเจ้าของร้าน จะเริ่มจากถามว่าตอนนี้เจอปัญหาอะไรกับซัพพลายเออร์เดิมบ้าง แทนที่จะเปิดด้วยการเสนอราคาทันที เพราะจากที่เรียนมา การถามให้ลูกค้าเล่าปัญหาก่อนทำให้เราเสนอของได้ตรงจุดกว่ามาก\n\n' +
    'อีกเรื่องที่ได้คือการแยกประเภทร้าน Stand alone กับ Chain ว่าตัดสินใจไม่เหมือนกัน ร้านเดี่ยวเจ้าของตัดสินใจเองจบในครั้งเดียว แต่ Chain ต้องผ่านฝ่ายจัดซื้อและใช้เวลานานกว่า ต้องวางแผนการติดตามต่างกัน\n\n' +
    'จุดที่ยังไม่มั่นใจคือเรื่องการต่อรองเมื่อลูกค้าเทียบราคากับเจ้าอื่นตรงๆ อยากได้เคสตัวอย่างเพิ่มเติมว่าควรตอบอย่างไรโดยไม่ต้องลดราคาทันที',

  // Medium — a couple of sentences, the most common shape.
  [`mock-sale-01_${C[1].id}`]:
    'ได้เทคนิคการปิดการขายที่ใช้ได้จริง โดยเฉพาะการสรุปสิ่งที่ลูกค้าพูดก่อนเสนอ ทำให้ลูกค้ารู้สึกว่าเราฟังจริงๆ จะลองใช้กับร้านที่ยังลังเลอยู่สัปดาห์หน้า',

  // Short — one line. Still has to render without looking broken.
  [`mock-sale-01_${C[2].id}`]: 'เข้าใจระบบ CRM มากขึ้น จะบันทึกทุกครั้งหลังออกพบลูกค้า',

  [`mock-sale-02_${C[0].id}`]:
    'ชอบส่วนที่อธิบายว่าทำไมร้านถึงเปลี่ยนซัพพลายเออร์ ส่วนใหญ่ไม่ใช่เพราะราคาแต่เพราะของขาดบ่อย ทำให้เข้าใจว่าทำไมเราต้องยืนยันสต็อกก่อนรับออเดอร์ทุกครั้ง',

  [`mock-sale-03_${C[0].id}`]:
    'ยังตามไม่ค่อยทันในส่วนการวิเคราะห์คู่แข่ง อยากขอทบทวนเรื่องการเปรียบเทียบราคากับเจ้าอื่นอีกรอบครับ',

  [`mock-tl-01_${C[0].id}`]:
    'ในฐานะหัวหน้าทีม สิ่งที่ได้คือกรอบคำถามที่จะใช้โค้ชน้องในทีมเวลาออกตลาดด้วยกัน จะเอาไปใช้ตอนทำ shadow visit รอบหน้า',
}
